...
// Terminal countdown settings
router.get("/terminal-countdown", async (req, res, next) => {
	try {
		const clientId = await resolveClientId(req);
		const val = await getClientSetting("terminal_countdown", clientId, "30");
		res.json({ seconds: Math.max(5, parseInt(val, 10) || 30) });
	} catch (err) {
		next(err);
	}
});
...
router.put("/terminal-countdown", requireAuth, async (req, res, next) => {
	try {
		const clientId = req.user.clientId || null;
		const seconds = clampInt(req.body.seconds, 5, 300, 30);
		await setClientSetting("terminal_countdown", String(seconds), clientId);
		await log(req, "settings.terminal_countdown", `seconds=${seconds}`);
		res.json({ seconds });
	} catch (err) {
		next(err);
	}
...
// Auto-open registration settings
router.get("/auto-open", requireAuth, async (req, res, next) => {
	try {
		const clientId = req.user.clientId || null;
		const enabled = await getClientSetting("auto_open_enabled", clientId, "0");
		const time = await getClientSetting("auto_open_time", clientId, "09:00");
		res.json({ enabled: enabled === "1", time: time || "09:00" });
	} catch (err) {
		next(err);
	}
});
...
router.put("/auto-open", requireAuth, async (req, res, next) => {
	try {
		...
		await setClientSetting("auto_open_time", timeVal, clientId);
		await log(
			req,
			"settings.auto_open",
			`${enabled ? "on" : "off"} at ${timeVal}`,
...
// Field minimum length settings
router.get("/field-min-length", async (req, res, next) => {
	try {
		const clientId = await resolveClientId(req);
		const val = await getClientSetting("field_min_length", clientId, "3");
		res.json({ min_length: Math.max(1, parseInt(val, 10) || 3) });
	} catch (err) {
		next(err);
	}
});
...
router.put("/field-min-length", requireAuth, async (req, res, next) => {
	try {
		const clientId = req.user.clientId || null;
		const min_length = clampInt(req.body.min_length, 1, 50, 3);
		await setClientSetting("field_min_length", String(min_length), clientId);
		await log(req, "settings.field_min_length", `min_length=${min_length}`);
		res.json({ min_length });
	} catch (err) {
		next(err);
	}
