    ...
    <div className="form-grid" style={{ padding: '0 0 8px' }}>
      <div className="form-row">
        ...
        <input className={`field ${errors.title ? 'field-error' : ''}`} value={form.title}
          onChange={e => onChange('title', e.target.value)} placeholder="Лендинг для ООО «Пример»" autoComplete="off" />
      </div>
      <div className="form-row">
        <label>Клиент</label>
        <input className="field" value={form.client_name}
          onChange={e => onChange('client_name', e.target.value)} placeholder="ООО «Пример»" autoComplete="off" />
      </div>
      <div className="form-row">
        <label>Услуга</label>
        <input className="field" value={form.service_name}
          onChange={e => onChange('service_name', e.target.value)} placeholder="Landing Page" autoComplete="off" />
      </div>
      <div className="form-row">
        <label>URL продакшена</label>
        <input className="field" value={form.prod_url}
          onChange={e => onChange('prod_url', e.target.value)} placeholder="https://example.ru" autoComplete="off" />
      </div>
      <div className="form-row" style={{ gridColumn: '1 / -1' }}>
        <label>Описание</label>
        <input className="field" value={form.description}
          onChange={e => onChange('description', e.target.value)} placeholder="Краткое описание проекта" autoComplete="off" />
      </div>
      <div className="form-row">
        <label>Статус</label>
        <select className="field" value={form.status} onChange={e => onChange('status', e.target.value)} autoComplete="off">
      ...
      <div className="form-row" style={{ gridColumn: '1 / -1' }}>
        ...
        <input className="field" value={form.notes}
          onChange={e => onChange('notes', e.target.value)} placeholder="Дополнительная информация" autoComplete="off" />
      </div>
    </div>
  )
}
