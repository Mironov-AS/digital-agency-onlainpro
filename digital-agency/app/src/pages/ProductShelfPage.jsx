...
import {
  ArrowLeft, Plus, Trash2, X, Check, Loader2,
  Package, Users, RefreshCw, ExternalLink, Play, Pause,
} from 'lucide-react'
import { apiFetch } from '../api.js'

export default function ProductShelfPage({ onBack, user }) {
  ...
  const [subForm, setSubForm] = useState({ product_code: '', client_id: '', billing_amount: '', billing_period: 'monthly', next_billing_date: '' })
  ...
  async function handleUpdateSubStatus(id, currentStatus) {
    const next = currentStatus === 'active' ? 'suspended' : currentStatus === 'suspended' ? 'active' : 'active'
    try {
  ...
  function openAddSubscription() {
    setSubForm({ product_code: '', client_id: '', billing_amount: '', billing_period: 'monthly', next_billing_date: '' })
    setSubError('')
  ...
  function handleSubProductChange(code) {
    ...
    setSubForm(f => ({
      ...f,
      product_code: code,
      billing_amount: prod ? String(prod.price_monthly || prod.price_yearly || '') : f.billing_amount,
      billing_period: prod ? (prod.billing_period || 'monthly') : f.billing_period,
    }))
  }
  ...
  function openEditProduct(product) {
    ...
    setEditForm({
      ...
      price_yearly: product.price_yearly != null ? String(product.price_yearly) : '',
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <div>
          <h1 className="page-title">Продуктовая полка</h1>
          <p className="page-sub">Цифровые продукты для клиентов</p>
        </div>
      ...
      {/* Sub-tabs */}
      <div className="section-tabs" style={{ padding: '0 32px', background: '#fff', borderBottom: '1px solid #e5e7eb' }}>
        <button
      ...
      {activeTab === 'products' && (
        <div style={{ padding: '24px 32px' }}>
          <div className="services-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
            {products.map(p => (
              <div key={p.id} className="svc-card" style={{ '--svc-color': '#7c3aed', '--svc-bg': '#f5f3ff', cursor: 'pointer' }}
                onClick={() => openEditProduct(p)}>
                <div className="svc-icon-wrap" style={{ background: '#f5f3ff', color: '#7c3aed' }}>
                  <Package size={28} />
                </div>
                <div className="svc-body">
                  ...
                  {p.config && p.config.backend_url && (
                    <div style={{ fontSize: 11, color: '#6b7280', marginTop: 8 }}>
                      <span style={{ fontFamily: 'monospace' }}>{p.config.backend_url}</span>
                    </div>
                  )}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 'auto', paddingTop: 12 }}>
                  ...
                  <span style={{ marginLeft: 'auto', fontSize: 12, color: '#9ca3af', fontFamily: 'monospace' }}>{p.code}</span>
                </div>
              </div>
            ))}
          </div>
                ...
                <tbody>
                  {subscriptions.map(s => {
                    ...
                    return (
                      <tr key={s.id}>
                        <td>
                          <div style={{ fontWeight: 600, color: '#111827' }}>{s.product_name || s.product_code}</div>
                          {s.product_description && (
                            <div style={{ fontSize: 12, color: '#6b7280' }} className="truncate" style={{ maxWidth: 300 }}>
                              {s.product_description}
                            </div>
                          )}
                        </td>
                        <td>
                          <div style={{ fontWeight: 500, color: '#374151' }}>{client?.name || s.client_id}</div>
                          {client?.email && <div style={{ fontSize: 12, color: '#6b7280' }}>{client.email}</div>}
                        </td>
                        <td style={{ fontWeight: 600 }}>
                          {s.billing_amount != null ? `${Number(s.billing_amount).toLocaleString('ru-RU')} ₽` : '—'}
                        </td>
                        <td style={{ color: '#6b7280', fontSize: 12 }}>
                          {s.billing_period === 'monthly' ? 'Ежемесячно' : s.billing_period === 'yearly' ? 'Ежегодно' : '—'}
                        </td>
                        <td style={{ color: '#6b7280', fontSize: 12 }}>
                          {s.next_billing_date ? new Date(s.next_billing_date).toLocaleDateString('ru-RU') : '—'}
                        </td>
                        <td>
                          <button
                            onClick={() => handleUpdateSubStatus(s.id, s.status)}
                            style={{
                              display: 'inline-flex', alignItems: 'center', gap: 4,
                              padding: '4px 10px', borderRadius: 6, fontSize: 12, fontWeight: 600,
                              cursor: 'pointer', border: 'none',
                              background: s.status === 'active' ? '#dcfce7' : s.status === 'trial' ? '#ede9fe' : s.status === 'suspended' ? '#fef9c3' : '#f3f4f6',
                              color: s.status === 'active' ? '#16a34a' : s.status === 'trial' ? '#7c3aed' : s.status === 'suspended' ? '#ca8a04' : '#6b7280',
                            }}
                          >
                            {s.status === 'active' ? <Play size={12} /> : s.status === 'trial' ? <Play size={12} /> : <Pause size={12} />}
                            {s.status === 'active' ? 'Активна' : s.status === 'trial' ? 'Триал' : s.status === 'suspended' ? 'Приостановлена' : 'Отменена'}
                          </button>
                        </td>
      ...
      {editProduct && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setEditProduct(null)}>
          <div className="modal-box" style={{ maxWidth: 520 }}>
            ...
            <form onSubmit={e => { e.preventDefault(); handleSaveProduct() }} className="modal-form" autoComplete="off">
              ...
              <div className="form-row" style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <input
                  ...
                  onChange={e => setEditForm(f => ({ ...f, show_on_landing: e.target.checked }))}
                  style={{ width: 18, height: 18 }}
                />
                <label htmlFor="show_on_landing" style={{ margin: 0, fontWeight: 500 }}>Показывать в меню «Продукты» на лендинге</label>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div className="form-row">
                  <label>Цена за месяц</label>
                  <input className="field" type="number" min="0" step="100" placeholder="5000"
                    value={editForm.price_monthly}
                    onChange={e => setEditForm(f => ({ ...f, price_monthly: e.target.value }))} autoComplete="off" />
                </div>
                <div className="form-row">
                  <label>Цена за год</label>
                  <input className="field" type="number" min="0" step="100" placeholder="50000"
                    value={editForm.price_yearly}
                    onChange={e => setEditForm(f => ({ ...f, price_yearly: e.target.value }))} autoComplete="off" />
                </div>
                <div className="form-row">
                  <label>Период по умолчанию</label>
                  <select className="field"
                    value={editForm.billing_period}
                    onChange={e => setEditForm(f => ({ ...f, billing_period: e.target.value }))} autoComplete="off">
              ...
              <div className="modal-actions">
                <button type="button" className="btn-cancel" onClick={() => setEditProduct(null)}>Отмена</button>
      ...
      {modal?.mode === 'addSubscription' && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="modal-box">
            ...
            <form onSubmit={e => { e.preventDefault(); handleAddSubscription() }} className="modal-form" autoComplete="off">
              <div className="form-row">
                <label>Продукт</label>
                <select
                  className="field"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div className="form-row">
                  ...
                  <input className="field" type="number" min="0" step="100"
                    placeholder="5000"
                    value={subForm.billing_amount}
                    onChange={e => setSubForm(f => ({ ...f, billing_amount: e.target.value }))} autoComplete="off" />
                </div>
                <div className="form-row">
                  <label>Периодичность</label>
                  <select className="field"
                    value={subForm.billing_period}
                    onChange={e => setSubForm(f => ({ ...f, billing_period: e.target.value }))} autoComplete="off">
                    <option value="monthly">Ежемесячно</option>
                    <option value="yearly">Ежегодно</option>
                  </select>
                </div>
                <div className="form-row">
                  ...
                  <input className="field" type="date"
                    ...
                    onChange={e => setSubForm(f => ({ ...f, next_billing_date: e.target.value }))} autoComplete="off" />
                </div>
              </div>
