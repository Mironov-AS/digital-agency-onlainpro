...
export function hasAccess(user, svc) {
  ...
  if (user.role === 'client') {
    // Клиент имеет доступ только к продуктам
    return svc.app === 'clients' && (svc.id === 'clients' || svc.id === 'product-shelf' || svc.id === 'analytics')
  }
  return user.apps?.includes(svc.app)
}

export function accessibleServices(user) {
  if (user.role === 'admin') return SERVICES
  if (user.role === 'client') {
    return SERVICES.filter(s => s.app === 'clients' && (s.id === 'clients' || s.id === 'product-shelf' || s.id === 'analytics'))
  }
