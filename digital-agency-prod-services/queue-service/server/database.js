...
async function initDb() {
  ...
  `);
	...
	// Migration 107: add require_check to service_fields (for duplicate detection on visitor kiosk)
	...
	if (!migV107) {
		try {
			const colInfo = await db.pool.query(
				"SELECT data_type FROM information_schema.columns WHERE table_name = 'service_fields' AND column_name = 'require_check'",
			);
			if (colInfo.rows.length === 0) {
				await db.exec(
					"ALTER TABLE service_fields ADD COLUMN require_check INTEGER DEFAULT 0 NOT NULL",
				);
			}
			await db.pool.query(
