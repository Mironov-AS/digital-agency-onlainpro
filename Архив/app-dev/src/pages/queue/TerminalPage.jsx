...
function ServiceSelection({ onSelect }) {
	...
	useEffect(() => {
		apiFetch("/api/settings/registration")
			...
			.then((d) => setRegOpen(d !== undefined ? (d.open ?? true) : true))
...
function ServiceForm({ service, onBack, onTicket }) {
	...
	useEffect(() => {
		apiFetch(`/api/services/${service.id}/fields`)
			.then((r) => {
				if (r) return r.json();
			})
			.then((data) => {
				...
				setFieldValues(init);
				if (arr.length > 0) setActiveFieldId(arr[0].id);
			})
			.catch(() => {});
	}, [service]);
	...
	const getTicket = async () => {
		...
		try {
			...
			if (hasCheckFields && fvArray.length > 0) {
				try {
					const checkRes = await apiFetch(
						"/api/service-fields/check-duplicate",
						{
							method: "POST",
							body: JSON.stringify({
					...
					if (checkRes) {
						const checkData =
							typeof checkRes === "object" && !Array.isArray(checkRes)
								? checkRes
								: {};
			...
			const data = typeof res === "object" && !Array.isArray(res) ? res : {};
			if (data.error) {
				setError(data.error);
				setLoading(false);
				return;
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-blue-700 to-blue-900 flex flex-col items-center justify-center p-6 pb-48">
...
function TicketCountdown({ ticket, onNewTicket }) {
	...
	useEffect(() => {
		apiFetch("/api/settings/terminal-countdown")
			...
			.then((d) => {
				const sec = d && d.seconds ? d.seconds : 30;
				setIntervalSec(sec);
				setTimeLeft(sec);
			})
			.catch(() => {
	...
	useEffect(() => {
		...
		const t = setTimeout(() => resetToServices(), 5000);
