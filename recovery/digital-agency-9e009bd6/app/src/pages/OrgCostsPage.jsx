    ...
    <div className="form-grid" style={{ padding: '0 0 8px' }}>
      <div className="form-row">
        ...
        <input className={`field ${errors.name ? 'field-error' : ''}`} value={form.name}
          onChange={e => onChange('name', e.target.value)} placeholder="Хостинг сервера" autoComplete="off" />
      </div>
      <div className="form-row">
        <label>Сумма (₽)</label>
        <input type="number" min="0" className={`field ${errors.amount ? 'field-error' : ''}`}
          value={form.amount} onChange={e => onChange('amount', e.target.value)} placeholder="0" autoComplete="off" />
      </div>
      <div className="form-row">
        <label>Периодичность</label>
        <select className="field" value={form.period} onChange={e => onChange('period', e.target.value)} autoComplete="off">
          {Object.entries(PERIOD_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
        </select>
      </div>
      <div className="form-row" style={{ gridColumn: '1 / -1' }}>
        <label>Примечание</label>
        <input className="field" value={form.note} onChange={e => onChange('note', e.target.value)} placeholder="Оплата за..." autoComplete="off" />
      </div>
    </div>
  )
}
