...
export default function ClientsPage({ onBack, onSelectClient }) {
	...
	async function handleSubmit() {
		...
		try {
			if (modal.mode === "add") {
				const created = await apiFetch(API_CLIENTS, {
					method: "POST",
					body: JSON.stringify(modal.form),
				});
				setClients((c) => [created, ...c]);
			} else {
				const updated = await apiFetch(`${API_CLIENTS}/${modal.target.id}`, {
					method: "PUT",
					body: JSON.stringify(modal.form),
				});
			...
			setModal((m) => ({
				...m,
				loading: false,
				serverError: err.error || "Ошибка сохранения",
			}));
		}
	}
	...
	async function handleDelete() {
		...
		try {
			...
			setModal((m) => ({
				...m,
				loading: false,
				serverError: err.error || "Ошибка удаления",
			}));
		}
	}
		...
		<div className="page">
			...
			{loading ? (
				...
				<div className="page-error">
					{error} <button onClick={loadClients}>Повторить</button>
				</div>
			) : filtered.length === 0 ? (
							...
							{filtered.map((c) => (
								...
								>
									<td>
										...
										{c.address && (
											<div className="svc-desc-sm">{c.address}</div>
										)}
									</td>
									<td>
										{c.contact_person && (
										...
										{c.phone && (
											<div
												style={{
													...
													fontSize: 13,
													color: "#374151",
													marginTop: c.contact_person ? 2 : 0,
												}}
											>
									...
									<td>
										...
										>
											<span
												title={
													c.is_online
														? `Онлайн пользователей: ${c.online_count || 0}`
											...
											<div>
												<div
													style={{
														fontSize: 13,
														fontWeight: 700,
														color: c.is_online ? "#15803d" : "#6b7280",
													}}
												>
													{c.is_online ? "Онлайн" : "Оффлайн"}
												</div>
												<div style={{ fontSize: 11, color: "#9ca3af" }}>
													{c.online_count || 0}/{c.total_users || 0}{" "}
													пользователей
												</div>
											</div>
										</div>
										...
										{c.services?.length > 0 ? (
											...
											>
												{c.services
													...
													.map((s) => {
														const isProduct = s._type === "product";
														const isTrial = s._status === "trial";
														const bg = isTrial
															? "#faf5ff"
															: isProduct
																? "#f5f3ff"
									...
									<td>
										{c.overdue_payments > 0 ? (
											<div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
												...
												>
													{c.overdue_payments} просрочено
												</span>
												<span style={{ fontSize: 11, color: "#991b1b" }}>
													{c.overdue_amount.toLocaleString('ru-RU')} ₽
												</span>
											</div>
									...
									<td>
										{c.status === "testing" ? (
											<span className="status-toggle status-toggle--testing">
												Тестирование
											</span>
										) : (
											<span
												className={`status-toggle ${c.is_active ? "status-toggle--on" : "status-toggle--off"}`}
											>
				...
				>
					<div className="modal-box">
						...
						>
							...
							/>
							{modal.serverError && (
								<div className="form-error">{modal.serverError}</div>
							)}
